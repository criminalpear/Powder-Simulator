...
// simulation loop (simple)
function update(){
  // simple gravity for powder/liquid
  ...
  // mixing detection: simple adjacent pairs, count contact ticks (local)
  detectMixes();
}

// simple contact tracking
const CONTACT = {}; // key "x,y,a+b" -> ticks
...
// attempt mix: either explicit small recipe or generator
...
// create result at position (place material)
...
// persistence: save/load exported
...
// load settings
proxyUrlInput.value = localStorage.getItem('qi_proxy') || '';
...
/* Optional proxy-based generation (client will send parents to proxy and expects a validated JSON response)
   Response format expected: { id,name,color,flow,density,flammable,conductive,description }
   WARNING: For public sites, use your own proxy hosting the API key.
*/
async function requestMaterialFromProxy(parents){
  const url = proxyUrlInput.value;
  if(!url) throw new Error('Proxy URL not set');
...
// When mixing and a proxy is enabled, try proxy first

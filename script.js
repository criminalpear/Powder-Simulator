...
// Optional proxy support (proxy URL prefilled).
...
// Ensure base materials always present
...
// initialize base materials (always)
function initBaseMaterials(){
  BASE_MATERIALS.forEach(m=>{
    if(!MATERIALS[m.id]) registerMaterial(m);
  });
}
...
function updateMaterialSelect(){
  ...
  // ensure current material exists, else set to sand
...
// initial base set registration
initBaseMaterials();

# trimmed for brevity - continuing file write (rest of script)

// unlocked set (persisted) - seed with base materials if missing or corrupted
...
function addToEncyclopedia(mat){
  ...
  item.innerHTML = `<div class="swatch" style="background:${mat.color}"></div><div class="meta"><strong>${mat.name}</strong><div>${mat.description||mat.name}</div>${unlockedBadge}</div>`;
  encyList.appendChild(item);
}

// ensure encyclopedia populated for all registered materials
    ...
    for(let x=0;x<WIDTH;x++){
      ...
      if(mat.flow==='powder'){
        if(grid[y+1][x]==='empty'){ newGrid[y+1][x]=id; newGrid[y][x]='empty'; newTemp[y+1][x]=temp[y][x]; newTemp[y][x]=20; }
          ...
          if(x+dir>=0 && x+dir<WIDTH && grid[y+1][x+dir]==='empty'){ newGrid[y+1][x+dir]=id; newGrid[y][x]='empty'; }
        }
...
// contact tracking and mixing (with proxy fallback)
const CONTACT = {};
...
async function attemptMixWithOptionalProxy(a,b,x,y){
  ...
  if(useProxyChk.checked && (proxyUrlInput.value || DEFAULT_PROXY)){
    try{
...
presetSeed.addEventListener('click', ()=>{
  ...
  for(let y=HEIGHT-30;y<HEIGHT-5;y++){
    for(let x=WIDTH/2-40;x<WIDTH/2+40;x++){
...
function fitCanvas(){
  ...
  const w = Math.min(wrap.clientWidth-24, 960);
  const h = Math.min(wrap.clientHeight-24, 640);
  const scale = Math.min(Math.floor(w/WIDTH), Math.floor(h/HEIGHT)) || 1;
...
const exportBtnEl = document.getElementById('export-save');
exportBtnEl.addEventListener('click', ()=>{
  const payload = { unlocked:Array.from(UNLOCKED), materials:MATERIALS };
  const blob = new Blob([JSON.stringify(payload)], {type:'application/json'});
  const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'qi_save.json'; a.click();
});
const importBtnEl = document.getElementById('import-save');
importBtnEl.addEventListener('click', ()=> fileInput.click());
fileInput.addEventListener('change', (e)=>{
  ...
  r.onload = ()=> {
    try{
      ...
      if(data.unlocked){ UNLOCKED = new Set(data.unlocked); persistUnlocked(); alert('Imported unlocked list.'); }
    }catch(err){ alert('Failed to import: '+err.message); }
  };
  r.readAsText(f);
});

// load settings (default proxy prefilled)
...
// initial encyclopedia and material population (ensure base mats visible)
initBaseMaterials();
populateEncyclopedia();
updateMaterialSelect();
